...
logger_1.logger.info("Connected to consul instance found over DNS: 'consul-client'");
var Docker = require('dockerode');
var docker = new Docker(dockerConnection);
logger_1.logger.info("Listening to Docker at connection " + JSON.stringify(dockerConnection));
...
function consulDeregister(svcName, svcId) {
    ...
    consul.agent.service.deregister(svcId, function (err) {
        if (err) {
            logger_1.logger.error("Couldn't deregister " + svcName + JSON.stringify(err));
        }
        else {
            logger_1.logger.info("DEREGISTERED " + svcName + " instance #" + svcId);
        }
    });
}
function syncRegistryWithContainerList() {
    ...
    consul.catalog.service.list(function (err, consulServices) {
        if (err) {
            logger_1.logger.error("Error getting services from Consul: " + JSON.stringify(err));
        }
        else {
            // b) list of running containers
            docker.listContainers({ all: true }, function (err, containers) {
                if (err) {
                    logger_1.logger.error("Error getting containers from Docker: " + JSON.stringify(err));
                }
                else {
                    var promises = [];
                    var allSvcs = [];
                    ...
                    logger_1.logger.info("Consul services known before sync: " + JSON.stringify(allSvcs));
                    ...
                    allSvcs.forEach(tSvc => {
                        ...
                        consul.catalog.service.nodes(tSvc, (err, nodesOfSvc) => {
                            logger_1.logger.info("CHECKING " + nodesOfSvc.length + "  instance(s) of known service " + tSvc);
                            if (err) {
                                console.error(err);
                            }
                            else {
                    ...
                    containers.forEach(itm => {
                        ...
                        if (allSvcs.indexOf(containerService) == -1) {
                            ...
                            if (itm.Labels["api-gateway-port"] && itm.State == "running") {
                                logger_1.logger.info("Found new service " + containerService);
                                var containerAttributes = readAttributesOfContainer(itm);
                                promises.push(consulRegister(containerAttributes.svcName, containerAttributes.svcAddress, containerAttributes.svcPort, ['image: ' + containerAttributes.svcImage, 'imageId: ' + containerAttributes.svcImageId], containerAttributes.svcId));
                            }
                        }
                    });
            ...
            containers.forEach(itm => {
                ...
                if (itm.Labels["api-gateway-port"] && itm.State == "running" &&
                    ...
                    if (!isKnown) {
                        ...
                        logger_1.logger.info("FOUND container '" + cSvc + "' -> is known in Consul #" + containerAttributes.svcId);
                    }
            ...
            if (evt.Type == "container") {
                if (evt.Actor.Attributes && evt.Actor.Attributes["api-gateway-port"]) {
                    ...
                    if (evt.status == 'start' && evt.Actor.Attributes && evt.Actor.Attributes["name"]) {
                        logger_1.logger.info("STARTED " + evt.Actor.Attributes["name"]);
                        var svcId = "27";
                        if (evt.Actor.ID) {
                        ...
                        container.inspect((err, data) => {
                            if (err) {
                                logger_1.logger.error("Could not get container info from docker for #" + svcId);
                            }
                            else {
                    ...
                    if (evt.status == 'stop' && evt.Actor.Attributes && evt.Actor.Attributes["name"]) {
                        logger_1.logger.info("STOPPED " + evt.Actor.Attributes["name"]);
...
//# sourceMappingURL=registrator-nodejs.js.map